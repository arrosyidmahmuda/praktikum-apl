...
void registerUser(User dataUser[], int &jumlahUser){
    ...
    while(true){
        try{
            ...
            if(dataUser[jumlahUser].username.empty()){
...
string login(User dataUser[], int jumlahUser){
    ...
    try{
        ...
        if(username.empty() || password.empty()){
            throw invalid_argument("Tidak boleh kosong!");
        }
    }
    catch(exception &e){
        cout << "\nError: " << e.what() << endl;
...
void tambahAlat(ptrAlat data, int *jumlah){
    ...
    try{
        ...
        if(!(cin >> (data + *jumlah)->jumlah)){
            throw invalid_argument("Jumlah harus angka!");
        }

        if((data + *jumlah)->jumlah < 0){
            throw out_of_range("Jumlah tidak boleh negatif!");
        }
    ...
    if (pilihJenis != 1 && pilihJenis !=2) {
        throw out_of_range("pilihannya 1 atau 2!");
    }
...
void beliAlat(ptrAlat data, int jumlah){
    ...
    try{
        ...
        if(beli <= 0){
            throw out_of_range("Jumlah beli harus lebih dari 0!");
        }
    }
    catch(exception &e){
        cout << "\nError: " << e.what() << endl;
        ...
        if(menu == 1){
            ...
            if(role == "admin"){
                ...
                do{
                    ...
                    try{
                        if(!(cin >> pilih)){
                            throw invalid_argument("Harus angka!");
                        }
                        cin.ignore();
                    }
                    catch(exception &e){
